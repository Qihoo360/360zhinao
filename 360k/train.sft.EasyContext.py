"""
- modified from https://github.com/jzhang38/EasyContext/blob/main/train.py
- SFT only differs in "args.sft" appearances (search for "args.sft")
- SFT loss reweighting could be simply derived to avoid mean reduction on GPUs with no targets
...
def main(args):
    if args.output_dir:
        args.output_dir += datetime.now().strftime("-%Y-%m-%d-%H_%M_%S")
    ...
    if args.sft:
        to_remove = [col for col in train_dataset.column_names if col not in ["input_ids", "target_ids"]]
    else:
    ...
    optim = DummyOptim(model.parameters(), lr=args.learning_rate, weight_decay=args.weight_decay)
    ...
    model.train()
    if args.sft:
        loss_func = CrossEntropyLoss(inplace_backward=True, reduction='sum')
    else:
    ...
    while True:
        for _, batch in enumerate(train_loader):
            if args.sft:
                ...
                target_ids = batch["target_ids"][..., : args.seq_length + 1][..., 1:]
            else:
                input_ids = batch["input_ids"][..., : args.seq_length + 1][..., :-1]
                target_ids = batch["input_ids"][..., : args.seq_length + 1][..., 1:]
            
            total_target_num = (target_ids != loss_func.ignore_index).sum().item()
            ...
            local_target_num = (local_target_ids != loss_func.ignore_index).sum().item()
            ...
            with accelerator.accumulate(model):
                ...
                # rescale loss to make each token equal weight
                # reduction='sum'后系数很简单，卡数除以总loss token数
                # 目前只有sft需要reweight loss
                if args.sft and local_target_num != 0:
                    reweight = accelerator.num_processes / total_target_num
                    # reweight = 1 / total_target_num
                    loss *= reweight
                    print(f"Process ID {accelerator.process_index}: local_target_num {local_target_num}, original loss {loss}, reweight {reweight}")

                accelerator.backward(loss)

                if accelerator.sync_gradients:
                    # follow https://huggingface.co/docs/accelerate/v0.30.0/en/package_reference/accelerator#accelerate.Accelerator.clip_grad_norm_, clip gradient here
                ...
                if accelerator.process_index == 0:
                    scheduler.step()
                optim.zero_grad()
                accelerator.print(f"Current lr: {scheduler.get_lr()}")
            ...
            if completed_steps % args.save_steps == 0:  # save every steps
...
if __name__ == "__main__":
    ...
    args.add_argument("--save-steps", type=int, default=100)
    args.add_argument("--warmup-steps", type=int, default=20)
    args.add_argument("--learning-rate", type=float, default=2e-5)
    args.add_argument("--max-grad-norm", type=float, default=1.0)
    args.add_argument("--weight-decay", type=float, default=0.1)
    ...
    args.add_argument("--sft", action="store_true")
    args.add_argument(
        "--parallel_mode",
        type=str,
